...
def test_fill(c, handle_missing):
    ...
    myobject.fred_compute_backtest(["RPI", "W875RX1", "IPDCONGD"], [0.2, 0.5, 0.3], [0 ,0 ,0])
...
@pytest.mark.parametrize('start_date', [datetime.datetime(1959,1,1), datetime.datetime(1959,2,1), datetime.datetime(2022,8,1)])        
def test_dates(c, start_date):
    ...
    assert math.isclose(myobject.stats.loc['Average Annual Return (%)'],returns.loc[start_date:,"RPI"].mean()*12,abs_tol=1e-3)
    assert math.isclose(myobject.stats.loc['Annualized Vol (%)'],returns.loc[start_date:,"RPI"].std()*np.sqrt(12),abs_tol=1e-3)
  
    
@pytest.mark.parametrize('end_date', [datetime.datetime(1959,3,1), datetime.datetime(2022,8,1)])        
def test_dates(c, end_date):
    ...
    myobject.fred_compute_backtest(["RPI"], [1.0], [0])
    assert math.isclose(myobject.stats.loc['Average Annual Return (%)'],returns.loc[:end_date,"RPI"].mean()*12,abs_tol=1e-3)
    assert math.isclose(myobject.stats.loc['Annualized Vol (%)'],returns.loc[:end_date,"RPI"].std()*np.sqrt(12),abs_tol=1e-3)
...
def test_backtest(c):
    ...
    assert myobject.stats.loc['Max DD from Base'] >= myobject.stats.loc['Max Drawdown (%)'] 
    assert math.isclose((returns[["RPI", "W875RX1", "IPDCONGD"]]*[0.2,0.5,0.3]).mean().sum()*12,myobject.stats.loc['Average Annual Return (%)'], abs_tol = 1e-3) 
    assert math.isclose((returns[["RPI", "W875RX1", "IPDCONGD"]]*[0.2,0.5,0.3]).sum(axis=1).std()*np.sqrt(12),myobject.stats.loc['Annualized Vol (%)'], abs_tol = 1e-3) 
