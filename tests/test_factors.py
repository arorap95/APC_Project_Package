...
def create_input():
    x = [datetime.datetime(2022,11,6), datetime.datetime(2022,11,7), datetime.datetime(2022,11,8), datetime.datetime(2022,11,9),
         datetime.datetime(2022,11,10), datetime.datetime(2022,11,11)]
...
@pytest.fixture
def c():
    def _c(df, standardization=2, maxfactors=None, factorselection=None, removeoutliers=None, handle_missing=1):
        return FredFactors(df,standardization, maxfactors, factorselection, removeoutliers, handle_missing)
...
def test_standardization(c, standardization):
    ...
    if standardization==0:
        assert np.array_equal(myobject.currentdata.iloc[:,0].values, scaleddata.iloc[:,0].values)
...
def test_optimal_factor(c, factorselection):
    ...
    if selection==0:
        additional_explained_variance = np.round((explained_variance_ratio), decimals=4)*100
        assert myobject.optimalfactors == np.argmax(additional_explained_variance<target)+1
