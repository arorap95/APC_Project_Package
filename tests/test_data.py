...
def test_stationarity(sample_data, transf_code):
    myobject = c()
    if transf_code == 1:
...
def test_vintage_start(vintage):
    """
...
def test_vintage_format(vintage):
    """
...
def test_duplicates(fred_qd_group):
    """
...
def test_interp_freq(date):
    ...
    """
    myobject_stationary = get_fred.GetFred(transform=True)
    myobject_levels = get_fred.GetFred(transform=False)
...
def test_md_group_varcount(group_no):
    """
...
def test_qd_group_varcount(group_no):
    """
    Determine the counts of variables from pulling the full dataset for a given
    group number is equal to the number of variables in the appendix for that group.
    Important to not lose variables.
