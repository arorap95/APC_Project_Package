...
class test_AR_Model(unittest.TestCase):
    ...
    def test__handle_missing(self, name, val):
    ...
    @parameterized.expand([["0", False, 2], ["1", True, 2]])
    def test_create_lagged_data(self, name, isSeries, maxlag):
        df = self.create_input()
        model = self.c()

        outp = model.create_lagged_data(df, maxlag, isSeries)
        if isSeries:
            outp_shape = (len(df.axes[1]), len(df), maxlag)
        else:
            outp_shape = (len(df), len(df.axes[1]) * maxlag)
        self.assertEqual(outp.shape, outp_shape)
