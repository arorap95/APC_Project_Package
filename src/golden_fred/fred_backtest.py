    ...
    def _run_backtest(self):
        ...
        for i in range(len(alldates)-1):
            ...
            for asset in prerebal.keys():
                prerebal[asset][alldates[i+1]] = postrebal[asset][alldates[i]] * (1+self.inputdata.loc[alldates[i]][asset])
                cumulativevalue[alldates[i+1]] +=   prerebal[asset][alldates[i+1]]        

            #apply rebalancing and turnover
            if self._is_rebal(i+1): 
                for asset in postrebal.keys():
                    postrebal[asset][alldates[i+1]]  = cumulativevalue[alldates[i+1]]*self.initialweights[asset]
                    currentturnover = abs(postrebal[asset][alldates[i+1]] -  prerebal[asset][alldates[i+1]])*self.Tcosts[asset]
                    postrebal[asset][alldates[i+1]] = postrebal[asset][alldates[i+1]] - currentturnover
                    turnover[alldates[i+1]] = turnover[alldates[i+1]] + currentturnover

                turnover[alldates[i+1]] = turnover[alldates[i+1]]/cumulativevalue[alldates[i+1]]
