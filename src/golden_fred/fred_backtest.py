...
class FredBacktest:
    def __init__(
        ...
        #-------------------------------------------------------------------------------------------------------
        assert(data.index[1].month-data.index[0].month==1), 'Must provide monthly Fred-MD data'
        
        assert handle_missing in [0,1], 'Handle Missing parameter must be an integer in [0,1]'
        assert rebalancing in ["monthly", "quarterly", "annually"], 'Rebalancing input not recognized'
        assert isinstance(data.index, pd.DatetimeIndex), 'Index of input data must be a datetime index'
        ...
        assert len(self.inputdata) > 2, 'Must have at least 3 data points to compute statistics'
        
        
    def fred_compute_backtest(
        self, factors: np.array, initialweights: np.array, Tcosts: np.array) -> pd.Series:
        ...
        assert len(initialweights)==len(factors), 'Length of weights must match length of factors'
        assert len(Tcosts)==len(factors), 'Length of T-costs must match length of factors'
        
        for factor in factors:
            assert factor in self.inputdata.columns.values, f'{factor} does not exist in the data column names'
    ...
    def regime_filtering(
        self, columns: np.array, lambda_param: np.array=None) -> pd.DataFrame:
       ...
       #Check that parameters are set correctly
        ...
        assert len(columns)>=1, 'Must have at least 1 column name for a historical regime backtest'
        assert len(lambda_param)==len(columns), 'Length of lambda parameters must match length of columns'
        
        for column in columns:
            assert column in self.inputdata.columns.values, f'{column} does not exist in the data column names'
    ...
    def _run_backtest(self):
        ...
        for i in range(len(alldates)-1):
            ...
            #apply returns
            prerebal[alldates[i+1]] = postrebal[alldates[i]] * (1+self.returndata.loc[alldates[i+1]][self.factors])
            cumulativevalue[alldates[i+1]] =   np.sum(prerebal[alldates[i+1]])       

            #apply rebalancing and turnover
            if self._is_rebal(i+1): 
                postrebal[alldates[i+1]]  = cumulativevalue[alldates[i+1]]*self.initialweights
                currentturnover = (abs(postrebal[alldates[i+1]] -  prerebal[alldates[i+1]])*self.Tcosts)
                postrebal[alldates[i+1]] = postrebal[alldates[i+1]] - currentturnover
                turnover[alldates[i+1]] = np.sum(currentturnover)/cumulativevalue[alldates[i+1]]
    ...
    def _run_regimefiltering(self):
        ...
        for i, column in enumerate(self.columns):
            ...
            x = cp.Variable(n)
            obj = cp.Minimize(.5* cp.sum_squares(y-x)+lambd * cp.norm(D @ x, 1))
            prob = cp.Problem(obj)
            prob.solve()
