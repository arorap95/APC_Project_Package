...
class FredRegression:
    ...
    def __init__(self, 
        ...
        if use_pca_features:
            assert fred_factors_kwargs!=None, "Please specify arguments for fred_factors"
            assert len(fred_factors_kwargs)==4,"Please specify all the 4 required arugments - standardization,maxfactors,factorselection,removeoutliers. Please refer to documentaion of class FredFactors in golden_fred/fred_factors for details."
            assert model_lags!=None, "Please provide lags for each window"
        if model_lags:
            assert len(model_lags)== round((end_date-start_date)/np.timedelta64(1,'M')), "length of model_lags provided should match the number of windows"
...
class AR_Model(FredRegression):
    ...
    def _split_train_test_fit(self, series_data, features, lag):
        ...
        return ar_model,out_sample_y_pred,in_sample_error,out_sample_error,out_sample_y
    ...
    def _model(self, series_data, features, lag=None):
        ...
        if self.find_ideal_lags:
            ...
            for lag in range(1, self.max_lag + 1):
                ar_model, out_sample_y_pred,in_sample_error,out_sample_error,out_sample_y = self._split_train_test_fit(series_data, features, lag)
