...
class FredFactors:
    ...
    def __init__(
        ...
        #-------------------------------------------------------------------------------------------------------
        assert standardization in [0,1,2], 'Standardization parameter must be an integer in [0,1,2]'
        if maxfactors is None:
            maxfactors = len(inputdata.columns)
        assert maxfactors <= len(inputdata.columns), 'Maximum factors cannot exceed number of columns'
        assert handle_missing in [0,1], 'Handle Missing parameter must be an integer in [0,1]'
        
        if factorselection is not None:
            assert len(factorselection) ==1, 'Can only specify one factor selection method'
            assert list(factorselection.keys())[0] in [0,1,2], 'Factor Selection Method must be in [0,1,2]'

            if list(factorselection.keys())[0] in [0,1]:
                assert (list(factorselection.values())[0]>=0 and list(factorselection.values())[0]<=100), 'Target variance must be between 0-100 percent'
                
            elif list(factorselection.keys())[0] in [2]:
                assert list(factorselection.values())[0] <= len(inputdata.columns)-2 , 'Maximum rank is number of columns - 2'
        #-------------------------------------------------------------------------------------------------------            
    ...
    def _removeoutliers(self):
        ...
        if self.removeoutliers:
            ...
            iqr = self.currentdata.quantile(0.75, axis=0) - self.currentdata.quantile(0.25, axis=0)
            iqr_limit = iqr*10
    ...
    def _selectoptimalfactors(self):
        ...
        if self.factorselection is not None:
            ...
            if selection==0:
                additional_explained_variance = np.round((self.explained_variance_ratio), decimals=4)*100
                optimalfactors = np.argmax(additional_explained_variance<target)+1

            elif selection==1:
                cumvar = np.cumsum(np.round(self.explained_variance_ratio, decimals=4)*100)
    ...
    def _fillmissing(self):
        ...
        self.currentdata.drop(index=self.currentdata.index[[0,1]], axis=0, inplace=True)
    ...
    def _runPCAalgorithm(self):
        ...
        unscaleddata = pd.DataFrame(columns = [f"Factor {i}" for i in range(1,len(pca.components_)+1)], data = unscaleddata)
        ...
        self.components = pd.DataFrame(columns = [f"Components {i}" for i in range(1,len(pca.components_)+1)], data = pca.components_)
