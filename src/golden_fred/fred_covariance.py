class CovarianceFred():
    ...
    def threshold_covariance(self, correlationthreshold = 0.01) -> pd.DataFrame:
        ...
        assert abs(correlationthreshold) <= 1, 'Correlation threshold must range from -1 to 1'
        
        self.covariancematrix = self.originaldata.cov()[abs(self.originaldata.corr())>correlationthreshold]
    ...
    def positive_semidefinite_method1(self, correlationthreshold = 0.0) -> pd.DataFrame:
        ...
        sample_covariance = self.originaldata.cov()[abs(self.originaldata.corr())>correlationthreshold]
    ...
    def positive_semidefinite_method2(self, correlationthreshold = 0.0) -> pd.DataFrame:
        ...
        g = max(-Lambda.min(),0)       
        self.covariancematrix = (sample_covariance + (g*np.identity(len(sample_covariance))))/(1+g)    
